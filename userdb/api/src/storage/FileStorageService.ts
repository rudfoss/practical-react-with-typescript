...
export class FileStorageService implements StorageService {
  ...
  protected constructor(
    public readonly options: Readonly<FileStorageServiceOptions>
  ) {}

  protected get<TDBItem extends keyof StorageData>(
    key: TDBItem,
    instanceFactory?: (
      data?: StorageData[TDBItem][number]
    ) => StorageData[TDBItem][number]
  ): StorageData[TDBItem] {
  ...
  protected async flushInactiveSessions() {
    const now = Date.now()
    await this.setUserSessions((sessions) =>
  ...
  protected async init() {
    ...
    this.db = await JSONFilePreset<StorageData>(
