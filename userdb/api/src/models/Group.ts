...
export class Group {
	public constructor(initialData?: Partial<ObjectFields<Group>>, validatorOptions?: ValidatorOptions) {
		if (initialData) {
...
export class NewGroup extends OmitType(Group, ["id", "roles"] as const) {
	@ApiProperty({
		...
		enum: UserDatabaseRole,
		description: "The roles assigned to users in this group. If not specified the Guest role will be added.",
		minLength: 1,
		required: false,
		isArray: true
	})
