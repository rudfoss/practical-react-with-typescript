...
import { UserDbApiClientsBaseClass as UserDatabaseApiClientsBaseClass } from "./UserDbApiClientsBaseClass"
...
const ApiClientsServiceContext = createContext<
  ApiClientsServiceContextProps | undefined
>(undefined)
...
export const ProvideApiClientsService = ({
  baseUrl,
  children
}: ProvideApiClientsServiceProps) => {
  const [sessionToken, setSessionTokenInternal] = useState<string>()
  const setSessionToken = useCallback((newSessionToken?: string) => {
    UserDatabaseApiClientsBaseClass.bearerToken = newSessionToken
  ...
  return (
    <ApiClientsServiceContext.Provider value={value}>
      {children}
    </ApiClientsServiceContext.Provider>
  )
}
